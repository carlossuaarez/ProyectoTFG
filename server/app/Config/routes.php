...
function registerRoutes(App $app, array $deps): void
{
    ...
    // Scheduler interno de notificaciones
    ...
    $app->group('/api', function ($group) use ($controllers, $limiters) {
        ...
        $group->post('/tournaments/{id:[0-9]+}/join', [$controllers['tournament'], 'join'])
            ->add($limiters['tournamentJoin']);

        // EQUIPOS
        $group->get('/tournaments/{id:[0-9]+}/teams', [$controllers['team'], 'getTournamentTeams'])
            ->add($limiters['tournamentDetail']);
        $group->post('/tournaments/{id:[0-9]+}/team-entry', [$controllers['team'], 'createTeamOrWaitlist'])
            ->add($limiters['tournamentJoin']);
        ...
        // PARTIDOS / RESULTADOS
        $group->get('/tournaments/{id:[0-9]+}/matches', [$controllers['match'], 'getTournamentMatches'])
            ->add($limiters['tournamentDetail']);
        $group->post('/tournaments/{id:[0-9]+}/matches/bootstrap', [$controllers['match'], 'bootstrapTournamentBracket'])
            ->add($limiters['tournamentJoin']);
